...

{
  ...
  "faceless_reason": "이 영상을 실제로 어떻게 만들 수 있는지 한 줄 (필요한 재료와 편집 방식 중심으로, 출연 여부 이야기는 하지 말 것)",
...
3) 모든 구간에 hold(그 구간에서 시청자가 나가지 않게 붙잡는 장치)를 반드시 쓰세요. 첫 구간은 선택한 전략의 후킹 장면입니다.\n4) narration은 수강생이 그대로 읽으면 되는 완성 문장으로 쓰세요. visual은 초보자가 그대로 찍고 편집할 수 있게 구체적으로 쓰세요 (누가/무엇이 화면에 어떻게 보이는지, 클로즈업·풀샷 같은 구도, 자막 위치까지).
...
아래 JSON 형식으로만 답하세요. 다른 텍스트 없이 JSON만. 모든 값은 초등학생도 이해할 쉬운 한국어로.
{
  ...
  "psychology": "이 컨셉이 시청자 머릿속에서 일으키는 일 — 왜 계속 보게 되는지 심리 해설 2~3문장",\n  "intent": "제작자가 이 구성으로 노리는 것 한두 문장",\n  "retention_devices": ["첫 3초 이후에도 시청자를 붙잡아두는 핵심 장치 2~4개"],\n  "timeline_basis": "대본 타임코드 | 추정",
  "length_sec": 30,
...
const TRANSCRIPT_PROMPT = `당신은 영상 대본 받아쓰기 전문가입니다. 이 영상을 처음부터 끝까지 직접 보고 들으며, 실제로 나오는 말을 한 글자도 빠짐없이 그대로 받아쓰세요.

가장 중요한 규칙 — 정확도 (반드시 지키세요):
- 들리는 말과 화면에 나오는 자막의 모든 단어를 하나도 빠뜨리지 말고 그대로 옮기세요.
- 요약·축약·의역·생략을 절대 하지 마세요. 문장을 짧게 줄이거나 단어를 빼지 마세요.
- 소리로 놓친 단어가 있으면 화면 자막을 보고 채워, 음성과 자막을 합쳐 완전한 문장으로 만드세요.
- 한 화자가 쉬지 않고 이어서 말하는 내용은 문장이 여러 개여도 끊지 말고 한 줄(text)에 전부 담으세요. 오직 화자가 바뀔 때만 줄을 나누세요.
- 영상 마지막 장면까지 모든 대사를 빠짐없이 담으세요.

그리고 각 줄이 누구의 말인지(화자)를 구분하세요.

아래 JSON 형식으로만 출력하세요:
{
  "language": "영상 음성 언어 (예: 한국어)",
  "voice_type": "AI음성 | 사람나레이션 | 원본소리 | 음악만 | 무음",
  "speakers": ["이 영상에 등장하는 화자 목록 (예: 나레이션, 아내, 남편, 시어머니)"],
  "lines": [ { "t": "0:03", "speaker": "화자 이름", "text": "한 화자가 이어서 하는 말 전체 (들리는 단어 그대로, 절대 줄이거나 자르지 말 것)" } ],
  "onscreen": ["화면에 표시되는 자막·텍스트 중 음성과 다른 것만 순서대로 (없으면 빈 배열)"],
  "note": "참고사항 한 줄 (예: 음성 없음 — 화면 자막만 추출)"
}

화자(speaker) 구분 규칙:
- 각 문장마다 누가 하는 말인지 speaker에 적으세요.
- 화면 속 인물, 목소리, 대화 속 호칭·맥락으로 역할이 분명하면 역할 이름으로 적으세요. 예: "할머니", "아빠", "여자아이", "아내", "남편", "시어머니".
- 한 사람이 여러 인물의 대사를 읽어주는 사연·이야기 형식이면, 그 대사가 이야기 속 누구의 말인지로 적고(예: "아내", "남편"), 상황을 설명하는 부분은 "나레이션"으로 적으세요.
- 실제로 여러 사람이 각자 말하면 목소리·모습으로 구분하세요.
- 누구인지 확신이 없으면 억지로 이름 붙이지 말고 "화자1", "화자2"처럼 번호로 적으세요.
- 같은 사람은 영상 전체에서 같은 이름/번호로 일관되게 적으세요.

- 나레이션이 없고 화면 자막만 있는 영상이면 lines에 화면 자막을 시간 순서대로 담고 speaker는 "자막"으로, note에 그 사실을 적으세요.
