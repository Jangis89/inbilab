...
def make_goldens(tmp):
    ...
    # clean 배경: 합성 2개 (패턴·그라데이션 — 감지 난이도 상이)
    for j, src_f in enumerate((
            f"testsrc2=size=1080x1920:rate={FPS},hue=H=t*20",
            f"gradients=size=1080x1920:rate={FPS}:speed=0.05,noise=alls=6:allf=t")):
        g = f"g{j+4:02d}"
        clean = os.path.join(tmp, f"{g}_clean.mp4")
        run(["ffmpeg", "-v", "error", "-f", "lavfi", "-i", src_f, "-t", str(DUR),
             "-c:v", "libx264", "-crf", "16", "-preset", "medium", clean, "-y"])
        burned = os.path.join(tmp, f"{g}_input.mp4")
        run(["ffmpeg", "-v", "error", "-i", clean, "-vf", drawtext(j + 3, *TEXTS[j + 3]),
             "-c:v", "libx264", "-crf", "16", "-preset", "medium", burned, "-y"])
        out.append((g, clean, burned, "gt-synth"))
